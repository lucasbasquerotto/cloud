...
def mix_inner(args):
  ...
  if shared_params:
    for key in shared_params:
      if (not shared_params_dict) or (key not in shared_params_dict):
        ...
        shared_params_expanded = merge_dicts(shared_params_expanded, params_aux)
  ...
  if shared_group_params:
    ...
    if shared_group_params_aux:
      info = expand_group_params(shared_group_params_aux, group_params_dict, 'shared_group_params')
      result_aux = info.get('result')
      error_msgs_aux = info.get('error_msgs')
