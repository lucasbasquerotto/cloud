...
The following are the parameters that can be specified when deploying a project with the `-n`/`--next` flag, specific of this layer:
...
| <nobr>`--printenv`</nobr> | Will print the environment content generated from the project environment file (and the project environment base file, if this was defined) and place it in a file at `<project_dir>/secrets/cloud/env.yml`. An optional parameter `--stdout` can be used to print the content stored in the file to `stdout`, and then removes the file. |
| <nobr>`--ssh`</nobr> | Will ssh into the host specified by the context (`-c`/`--ctx`), node type (`-n`/`--node`) and index (`-i`/`--idx`), with these params specified right after `--ssh`. When the context is not specified, if there is only one context in the deployment, this context will be used by default, otherwise an error will be thrown. When the node type is not specified, if there is only one node type in the context, this node type will be used by default, otherwise an error will be thrown. When the index is not specified, the default will be `1` (the first host of the previously mentioned node type). This ssh option can only be used after the preparation step is completed at least once, and the hosts are defined in the hosts file (either directly or after a node service creates them). To automatically accept the SSH key fingerprint, use the option `--skip-host-check` (`-h`). |
